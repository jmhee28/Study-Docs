...
- 단점(공정성 관점): 여러 토픽을 함께 구독할 때 **특정 consumer에 여러 토픽의 앞 구간이 몰리는 형태**가 나와서 *토픽 공정성*이 깨질 수 있어.
...
- 의미: 기존 “eager(한 번에 다 반납 → 재할당)” 방식처럼 *전원이 동시에 멈추는 stop-the-world*를 줄이고, **일부 partition만 단계적으로 이동**시켜 다운타임/스톨을 완화하는 쪽. (Incremental cooperative rebalance는 KIP-429에서 정의) ([cwiki.apache.org](https://cwiki.apache.org/confluence/x/vAclBg?utm_source=chatgpt.com))
